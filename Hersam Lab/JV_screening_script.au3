...
If Not WinActivate("KE2400") Then
   ...
   ; Open the KE2400 IV measurement tool
   Send("^o")
...
For $i = 0 To UBound($filter_positions)-1
   ...
   If $filter_positions[$i] = 1 Then
	  $partial_path = $data_dir&$device_name&" JV Data\"&$device_name&"_JVscreen_1sun"
   ElseIf $filter_positions[$i] = 12 Then
	  $partial_path = $data_dir&$device_name&" JV Data\"&$device_name&"_JVscreen_dark"
   Else
	  $partial_path = $data_dir&$device_name&" JV Data\"&$device_name&"_JVscreen_"&$filters[$filter_positions[$i]-1]&"OD"
   EndIf
...
For $i = 0 To UBound($filter_positions)-1
   ; Wait 2 sec before trying to plot the next curve
	...
	If $filter_positions[$i] = 1 Then
		Send('FEDMS_PlotJV("'&$device_name&'","JVscreen","1sun"){ENTER}')
	ElseIf $filter_positions[$i] = 12 Then
		Send('FEDMS_PlotJV("'&$device_name&'","JVscreen","dark"){ENTER}')
	Else
